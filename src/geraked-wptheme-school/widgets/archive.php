 ...
 */
class sch_ztjalali_archive extends WP_Widget {

    function __construct() {
        parent::__construct(
                'sch_ztjalali_archive', __('sch-archive', 'ztjalali'), array('description' => __('jalali archive widget', 'ztjalali'),)
        );
    }
    ...
    public function form($instance) {
        ...
        if (!isset($instance['jarchive_icon']))
            $instance['jarchive_icon'] = 'fa-archive';
        ...
        <div dir="rtl" align="justify">
            <p style="text-align:right">
                <label for="<?php echo $this->get_field_id('jarchive_title'); ?>"><?php _e('title', 'ztjalali') ?>:</label>
				<input class="widefat" id="<?php echo $this->get_field_id('jarchive_title'); ?>" name="<?php echo $this->get_field_name('jarchive_title'); ?>" type="text" value="<?php echo $instance['jarchive_title']; ?>" />
            </p>
            <p style="text-align:right">
                <label for="<?php echo $this->get_field_id('jarchive_icon'); ?>"><?php _e('آیکن', 'ztjalali') ?>: </label>
				<input class="widefat" style="direction:ltr;" id="<?php echo $this->get_field_id('jarchive_icon'); ?>" name="<?php echo $this->get_field_name('jarchive_icon'); ?>" type="text" value="<?php echo $instance['jarchive_icon']; ?>" />
            </p>
            <p style="text-align:right">
            ...
            <p style="text-align:right">
                <label for="<?php echo $this->get_field_id('jarchive_link_count'); ?>"><?php _e('link count(zero to unlimit)', 'ztjalali') ?>: </label>
				<input class="widefat" style="direction:ltr;" onkeypress="return isNumberKey(event)" id="<?php echo $this->get_field_id('jarchive_link_count'); ?>" name="<?php echo $this->get_field_name('jarchive_link_count'); ?>" type="text" value="<?php echo $instance['jarchive_link_count']; ?>" />
            </p>
            <p style="text-align:right">
    ...
    public function update($new_instance, $old_instance) {
        ...
        $instance['jarchive_icon'] = strip_tags($new_instance['jarchive_icon']);
    ...
    public function widget($args, $instance) {
        ...
        extract($args);
        echo '<div class="panel panel-default box links">';
        echo '<div class="panel-heading foot-title"><i class="fa '.$instance['jarchive_icon'].'"></i><h4 class="panel-title">' . $instance['jarchive_title'] . '</h4></div><div class="panel-body">';
        if ($instance['jarchive_dropdown']) {
            ...
            sch_ztjalali_archive_widget($instance['jarchive_type'],'option',$instance['jarchive_show_post_count'],$instance['jarchive_link_count']);
            echo "</select>";
        } else {
            echo '<ul class="foot-archive">';
            sch_ztjalali_archive_widget($instance['jarchive_type'],'html',$instance['jarchive_show_post_count'],$instance['jarchive_link_count']);
            echo '</ul>';
        }
 ...
 */
add_action('widgets_init', 'register_sch_ztjalali_archive');

function register_sch_ztjalali_archive() {
    register_widget('sch_ztjalali_archive');
}
 ...
 * own widget function
 */
function sch_ztjalali_archive_widget($type ='monthly',$format='html',$show_post_count=false,$limit=12,$before='',$after='') {
    ...
    if ($type === "yearly") {
        ...
        if (!empty($YearlyQry)) {
            ...
            foreach ($jYears as $jYear =>$data) {
                ...
                echo get_archives_link($url, '<span style="font-size:1px;color:white;">ء</span>'.$jYear.$c_after, $format, $before, $after);
                if ($i == $limit)
                    break;
                $i++;
            }
        ...
        if (!empty($MonthlyQry)) {
            ...
            foreach ($jMonths as $jMonth =>$data) {
                ...
                $c_after = $show_post_count ? '&nbsp;(' . ztjalali_persian_num($data['count']) . ')' . $after : '';
                echo get_archives_link($url, $jM.' '.$jY.$c_after, $format, $before, $after);
                if ($i == $limit)
                    break;
                $i++;
            }
        ...
        if (!empty($DailyQry)) {
            foreach ($DailyQry as $Daily) {
                ...
                echo get_archives_link($url, $text.$c_after, $format, $before, $after);
            }
        }
