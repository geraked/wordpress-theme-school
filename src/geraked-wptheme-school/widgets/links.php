<?php
/**
 * Widget API: sch_WP_Widget_Links class
 *
 * @package WordPress
 * @subpackage Widgets
 * @since 4.4.0
 ...
 * @see WP_Widget
 */
class sch_WP_Widget_Links extends WP_Widget {
	...
	public function __construct() {
		...
		parent::__construct('links', __('sch-links'), $widget_ops);
	}
	...
	public function widget( $args, $instance ) {
		...
		$icon = !empty( $instance['icon'] ) ? $instance['icon'] : 'fa-link';

		$before_widget = preg_replace( '/id="[^"]*"/', 'id="%id"', '<div class="panel panel-default box links foot-other">' );

		$widget_links_args = array(
			'title_before'     => '<div class="panel-heading foot-title"><i class="fa '.$icon.'"></i><h4>',
			'title_after'      => '</h4></div><div class="panel-body">',
...
function sch_load_links() {
	register_widget( 'sch_WP_Widget_Links' );
}
add_action( 'widgets_init', 'sch_load_links' );
?>
