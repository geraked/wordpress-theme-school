...
function sch_load_statistics() {
	register_widget( 'Sch_WP_Statistics_Widget' );
}
add_action( 'widgets_init', 'sch_load_statistics' );


class Sch_WP_Statistics_Widget extends WP_Widget {
	...
	public function __construct() {
		parent::__construct(
			'Sch_WP_Statistics_Widget', // Base ID
			__( 'sch-statistics', 'wp-statistics' ), // Name
	...
	public function widget( $args, $instance ) {
		...
		if (empty($widget_options['name_widget']))
			$widget_options['name_widget'] = 'آمار بازدید';

		if (empty($widget_options['icon_widget']))
			$widget_options['icon_widget'] = 'fa-sellsy';

		echo '<div class="panel panel-default box visits">';
		echo '<div class="panel-heading foot-title"><i class="fa '.$widget_options['icon_widget'].'"></i><h4 class="panel-title">' . $widget_options['name_widget'] . '</h4></div><div class="panel-body">';
		echo '<ul class="foot-visit">';

		if ( $widget_options['useronline_widget'] ) {
			echo '<li><i class="fa fa-desktop"></i>';
		...
		if ( $widget_options['tvisit_widget'] ) {
			echo '<li><i class="fa fa-bar-chart"></i>';
		...
		if ( $widget_options['yvisit_widget'] ) {
			echo '<li><i class="fa fa-area-chart"></i>';
		...
		if ( $widget_options['wvisit_widget'] ) {
			echo '<li><i class="fa fa-line-chart"></i>';
		...
		if ( $widget_options['ttvisit_widget'] ) {
			echo '<li><i class="fa fa-pie-chart"></i>';
		...
		if ( $widget_options['ser_widget'] ) {
			...
			echo '<li><i class="fa fa-search"></i>';
		...
		if ( $widget_options['tp_widget'] ) {
			echo '<li><i class="fa fa-file-text-o"></i>';
	...
	public function form( $instance ) {
		...
		<p>
			<label for="icon_widget"><?php _e( 'آیکن', 'wp-statistics' ); ?>:
				<input id="icon_widget" name="icon_widget" type="text"
					   style="direction:ltr;" value="<?php echo $widget_options['icon_widget']; ?>"/>
			</label>
		</p>
