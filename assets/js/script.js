$(document).ready(function () {
    ...
    if ($("#wpadminbar").length) {
        if ($(window).width() > 781)
            $("#custom-bootstrap-menu.navbar").css("top", "31px");
        else if ($(window).width() > 600)
            $("#custom-bootstrap-menu.navbar").css("top", "47px");
        else
            $("#custom-bootstrap-menu.navbar").css("top", "0");
...
//----------------------Number Convertor--------------------------
function convertDigitsToPersian(dom) {
    const persian = { 0: '۰', 1: '۱', 2: '۲', 3: '۳', 4: '۴', 5: '۵', 6: '۶', 7: '۷', 8: '۸', 9: '۹' };
    const ignore = ['script', 'style', 'link', 'input', 'textarea', 'pre', 'canvas', 'svg', 'video', 'audio'];

    if (!dom)
        return;

    function traverse(el) {
        if (ignore.includes(el.nodeName.toLowerCase())) return;
        if (el.nodeType == 3) {
...
function convertDigitsToEnglish(dom) {
 ...
 */
(() => {
    let labels = document.querySelectorAll('#top-footer li label');
    let other, ul, title;

    for (let label of labels) {
        if (label.innerHTML.includes('بازدید')) {
            ul = label.parentElement.parentElement;
            other = ul.parentElement;
            title = other.querySelector('.foot-title');
            break;
        }
    }
